    ...
    <div className="flex-1 min-w-0">
      ...
      >
        ...
        <div className="flex items-center">
          ...
          {!isLast && (
            <div className={`flex-1 h-0.5 ${lineColor} ${!hasDecision ? 'opacity-40' : ''}`} />
          )}
        </div>
        ...
        <div className="mt-2 pr-2">
          ...
          {hasDecision ? (
            <p className={`text-sm ${correct ? 'text-emerald-400' : 'text-red-400'}`}>
              {formatAction(decision.heroAction, decision.heroAmount)}
            </p>
          ) : (
      ...
      {expanded && decision && (
        <div className="mt-2 p-3 bg-slate-700/50 rounded-lg text-sm space-y-1">
          <p className="text-slate-300">
            ...
            {formatAction(decision.optimalAction, decision.optimalAmount)}
          </p>
          <div className="text-slate-400">
            <p>Fold: {Math.round(decision.optimalFrequencies.fold * 100)}%</p>
...
export function HandTimeline({ decisions }: HandTimelineProps) {
  ...
  return (
    <div className="bg-slate-800 rounded-xl p-6 shadow-lg">
      <h3 className="text-lg font-semibold text-slate-100 mb-4">Hand Timeline</h3>
    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1 pb-3">
        <button
          ...
          disabled={!hasDecision}
        >
          <p className="text-sm font-medium text-slate-300">{ROUND_LABELS[round]}</p>
          {hasDecision ? (
            <p className={`text-sm ${correct ? 'text-emerald-400' : 'text-red-400'}`}>
              {formatAction(decision.heroAction, decision.heroAmount)}
            </p>
