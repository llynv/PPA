...
function heroActionToKey(action: string): ActionKey {
  switch (action) {
    ...
    case 'check': return 'Call'; // check maps to call bucket
    case 'bet': return 'Raise';  // bet maps to raise bucket
  ...
  if (chartData.length === 0) {
    return (
      <div className="bg-slate-800 rounded-xl p-6 shadow-lg">
        <h3 className="text-lg font-semibold text-slate-100 mb-4">Optimal Frequencies</h3>
        <p className="text-slate-400 text-sm">No decisions to display.</p>
      </div>
    );
  }
      ...
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={chartData} barCategoryGap="20%">
          ...
          {(['Fold', 'Call', 'Raise'] as const).map((key) => (
            <Bar key={key} dataKey={key} name={key} stackId={undefined}>
              {chartData.map((entry, index) => (
                <Cell
                  key={index}
                  fill={ACTION_COLORS[key]}
                  opacity={entry.heroKey === key ? 1 : 0.5}
                  stroke={entry.heroKey === key ? '#ffffff' : 'none'}
