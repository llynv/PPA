        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {OPPONENT_COUNTS.map((n) => (
              <button
                key={n}
                onClick={() => updateSettings({ playerCount: n + 1 })}
            ...
            {BLIND_PRESETS.map((preset) => (
              <button
                ...
                className={`${buttonBase} ${
                  settings.smallBlind === preset.smallBlind &&
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {STACK_PRESETS.map((stack) => (
              <button
                key={stack}
                onClick={() => updateSettings({ startingStack: stack })}
                className={`${buttonBase} ${
                  settings.startingStack === stack ? activeClass : inactiveClass
                }`}
              >
